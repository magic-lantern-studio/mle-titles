...
Android Runtime Engine. Keep in mind that Java uses a Big Endian file format; therefore, we
...
1. Change working directory to:
	cd M:\projects\MagicLantern\Titles\dppTest\android\com.wizzer.mle.title.dpptest\src

2. Generate the Actor Group chunks:
	gengroup -b -j gen -d gen android DppImageTest.dwp ActorID.java GroupID.java
...
3. Generate the Scene chunks:
	genscene -b -j gen -d gen android DppImageTest.dwp SceneID.java
...
4. Generate the MediaRef chunks:
	genmedia -b -d gen android DppImageTest.dwp DppImageTest.BOM
...
5. Generate the Runtime tables:
	gentables -j gen -d gen android DppImageTest.dwp
...
6. Generate the DPP layout TCL script:
	genppscript -d gen android DppImageTest.dwp DppImageTest.dpp DppImageTest.tcl DppTOC

	Note: this will generate the TCL layout script used by the gendpp utility.
	It will be placed in the gen subdirectory.

7. Generate the DPP:
	gendpp -b -j gen -s gen -d gen android DppImageTest.dwp gen\\DppImageTest.tcl
...
8. Move the Digital Playprint, DppImageTest.dpp, to the ../res/raw/playprint.dpp file and refresh
   the Eclipse project.
